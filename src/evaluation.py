...
# Keep evaluation self-contained: avoid importing src/config.py (it may require optional deps).
SRC_DIR = Path(__file__).resolve().parent
ROOT_DIR = SRC_DIR.parent
FIX_INFO = str(ROOT_DIR / "data" / "fix_info.csv")
QUERIES_PATH = str(SRC_DIR / "queries")
CODEQL_PATH = os.environ.get("CODEQL_PATH", "")


def _codeql_bin() -> str:
    """Return the CodeQL CLI path.

    Prefer `CODEQL_PATH` (env/config) to avoid relying on PATH.
    """
    return CODEQL_PATH or "codeql"
...
class QueryEvaluator:
    ...
    def _extract_fixed_locations(self) -> Dict[str, Set[str]]:
        ...
        Uses `data/fix_info.csv` and avoids pandas dependency.
        """
        fixed_files: Set[str] = set()
        fixed_methods: Set[str] = set()

        if not os.path.exists(self.fix_data):
            self.logger.warning(f"FIX_INFO not found: {self.fix_data}")
            return {"files": fixed_files, "methods": fixed_methods}

        with open(self.fix_data, "r", encoding="utf-8", errors="replace", newline="") as f:
            reader = csv.DictReader(f)
            for row in reader:
                if (row.get("cve_id") or "").strip() != self.cve_id:
                    continue
                ...
                mtd = (row.get("method") or "").strip()
                if cls and mtd:
                    fixed_methods.add(f"{file_name}:{cls}:{mtd}")

        self.logger.info(
            f"Extracted {len(fixed_files)} fixed files and {len(fixed_methods)} fixed methods for {self.cve_id}"
        )
        return {"files": fixed_files, "methods": fixed_methods}
    
    
    def _load_project_structure(self) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        """Load project classes and methods using CodeQL queries with caching.

        Returns two lists of dicts with keys: file,name,start_line,end_line.
        """
        ...
        def _read_loc_csv(path: Path) -> List[Dict[str, Any]]:
            ...
            with open(path, "r", encoding="utf-8", errors="replace", newline="") as f:
                reader = csv.DictReader(f)
                for r in reader:
                    file_v = (r.get("file") or r.get("sourceFile") or r.get("path") or "").strip()
                    name_v = (r.get("name") or r.get("qualifiedName") or "").strip()
                    try:
                        start_v = int((r.get("start_line") or r.get("startLine") or "0") or 0)
                        end_v = int((r.get("end_line") or r.get("endLine") or "0") or 0)
                    except Exception:
                        continue
                    if not file_v or not name_v:
                        continue
                    rows.append({"file": file_v, "name": name_v, "start_line": start_v, "end_line": end_v})
            return rows
        ...
        if classes_cache.exists() and methods_cache.exists():
            try:
                ...
                project_classes = _read_loc_csv(classes_cache)
                project_methods = _read_loc_csv(methods_cache)
            ...
            for thread_flow in thread_flows:
                ...
                for location in locations:
                    try:
                        ...
                        # Get the closest enclosing class/method using database file path
                        relevant_classes = [
                            c
                            for c in (self.project_classes or [])
                            if c.get("file") == db_file_name
                            and int(c.get("start_line", 0)) <= int(start_line)
                            and int(c.get("end_line", 0)) >= int(start_line)
                        ]
                        if not relevant_classes:
                            continue
                        relevant_class = sorted(relevant_classes, key=lambda r: int(r.get("start_line", 0)), reverse=True)[0].get("name")

                        relevant_methods = [
                            m
                            for m in (self.project_methods or [])
                            if m.get("file") == db_file_name
                            and int(m.get("start_line", 0)) <= int(start_line)
                            and int(m.get("end_line", 0)) >= int(start_line)
                        ]
                        if not relevant_methods:
                            continue
                        relevant_method = sorted(relevant_methods, key=lambda r: int(r.get("start_line", 0)), reverse=True)[0].get("name")
